...
class my_network():
    ...
    def train_ANN(self):
        ...
        filepath = "Save_Model/checkpoint_ANN_GA_Sn.hdf5"
        ...
        err = [(y_true[i] - y_pred[i]) for i in range(len(y_true))]
        index_n = [i for i in range(len(y_true))]
        df = pd.DataFrame({'y_true': y_true, 'y_pred': y_pred, 'error(cm)': err}, index=index_n)
        print(df)

        regression_test = LinearRegression(fit_intercept=False).fit(y_true_test, y_predict_test)
        R2e = r2_score(y_true_test, y_predict_test)
        R = str(np.sqrt(R2e))[:6]
        plt.text(20, 7.5, "R = " + R, color='red')
        plt.scatter(self.y_test, y_predict_test)
        plt.plot(y_true_test, regression_test.predict(y_true_test), color='red', linewidth=3)
        plt.title('Sn Predict vs True - TEST DATA - Final')
        plt.xlabel('True')
        plt.ylabel('Predict')
        ...
        plt.title('Sn Predict vs True - ALL DATA - Final')
        plt.xlabel('True')
        plt.ylabel('Predict')
        ...
        plt.ylabel('Do sut Sn (cm)')
        plt.xlabel('So luong mau test')
        plt.title('So sanh Sn_thucte va Sn_dudoan - Total')
        ...
        ax1.set_xlabel('Error (cm)', fontsize=10)
...
def main():
    # load data
    df = pd.read_csv('Data/Do_sut_2.csv')
    # df.info()
    # print(df.describe())
    # print(df.isnull().sum())
    ...
    pickle.dump(sc, open('Save_Model/scaler_ANN_GA_Sn.pkl', 'wb'))
